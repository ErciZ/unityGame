...
public class MonsterBaseActionState : MonsterBaseState {
    /// <summary>
    /// 有限状态机状态初始化时调用。
    /// </summary>
    ...
    private void OnDeadEvent(IFsm<Monster> fsm, object sender, object userData) {
        int entityId = (int)userData;

        Log.Info("Monster OnDeadEvent ， entityId：" + entityId);

        if (entityId == fsm.Owner.Id) {
            ChangeState<MonsterDeadState>(fsm);
        }
    }
}